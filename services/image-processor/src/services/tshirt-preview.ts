...
export const generateTshirtPreview = async (
	...
	try {
		...
		try {
			...
			// テンプレートが存在しない場合は白を使用
			logger.warn(
				`テンプレート ${color}.png が見つかりません。白を使用します。`,
			);
			color = "white";
		}
...
export const normalizeColor = (colorText: string): string => {
	...
	if (lowerColor.includes("white") || lowerColor.includes("ホワイト"))
		return "white";
	if (lowerColor.includes("black") || lowerColor.includes("ブラック"))
		return "black";
	if (lowerColor.includes("navy") || lowerColor.includes("ネイビー"))
		return "navy";
