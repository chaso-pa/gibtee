...
import { generateTshirtPreview, normalizeColor, normalizeSize } from '../services/tshirt-preview.js';
...
export const convertImage = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const convertedImageBuffer = await withRetry(() => convertToGhibliStyle(resizedBuffer));

    // 変換された画像をS3にアップロード
    const userPrefix = userId ? `users/${userId}/images/` : 'images/';
    const convertedImageKey = await uploadBufferToS3(convertedImageBuffer, 'ghibli_converted.jpg', userPrefix);
 ...
 * Tシャツプレビューを生成する
 */
export const generateTshirtPreviewController = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Tシャツプレビューを生成
    const previewBuffer = await generateTshirtPreview(imageBuffer, normalizedColor, normalizedSize);
