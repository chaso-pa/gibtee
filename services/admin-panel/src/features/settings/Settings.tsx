...
export const Settings: React.FC = () => {
  ...
  const fetchSettings = async () => {
    try {
      setLoading(true);
      const res = (await api.get('/api/admin/settings')) satisfies SystemSettings;
      // @ts-ignore
              ...
              <FormControl>
                <HStack justify='space-between'>
                  ...
                  <Switch
                    ...
                    onChange={(e) => setIsOrderAcceptanceEnabled(e.target.checked)}
                    colorScheme='blue'
                    size='lg'
                  />
                </HStack>
            ...
            <CardBody>
              <VStack spacing={3} align='start'>
                <HStack>
                  ...
                  <Text>{new Date(settings.updatedAt).toLocaleString('ja-JP')}</Text>
                </HStack>
                <HStack>
                  <Text fontWeight='medium' minW='100px'>
                    作成日時:
                  </Text>
                  <Text>{new Date(settings.createdAt).toLocaleString('ja-JP')}</Text>
                </HStack>
              </VStack>
            </CardBody>
          </Card>
