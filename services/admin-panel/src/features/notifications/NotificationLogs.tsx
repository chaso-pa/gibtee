...
const NotificationLogs = () => {
  ...
  const getTypeStyle = (type) => {
    switch (type) {
      case 'STATUS_UPDATE':
        return { colorScheme: 'blue', icon: <RepeatIcon mr={1} />, label: 'ステータス通知' };
      case 'SHIPPING_UPDATE':
        return { colorScheme: 'teal', icon: <BellIcon mr={1} />, label: '発送通知' };
      case 'ORDER_REMINDER':
        return { colorScheme: 'purple', icon: <TimeIcon mr={1} />, label: '注文リマインダー' };
      case 'PAYMENT_REMINDER':
        return { colorScheme: 'orange', icon: <InfoIcon mr={1} />, label: '支払いリマインダー' };
      default:
  ...
  return (
    <Box p={5}>
      {/* ヘッダー */}
      <Heading size="lg" mb={5}>通知ログ管理</Heading>
