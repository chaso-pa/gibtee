...
export const createLinePayRequest = async (
	...
	try {
		...
		if (responseData.returnCode === "0000") {
			...
			logger.info(
				`LINE Pay決済リクエスト成功: transactionId=${responseData.info.transactionId}`,
			);
			return {
			...
			throw new Error(
				`LINE Pay APIエラー: ${responseData.returnCode} - ${responseData.returnMessage}`,
			);
		}
	} catch (error: any) {
 ...
 * LINE Pay決済を確定する
 */
export const confirmLinePayPayment = async (
	...
	try {
		...
		if (responseData.returnCode === "0000") {
			...
			throw new Error(
				`LINE Pay確定APIエラー: ${responseData.returnCode} - ${responseData.returnMessage}`,
			);
		}
	} catch (error: any) {
...
export const createStripeCheckoutSession = async (
	...
	try {
		logger.info(
			`Stripe決済セッション作成開始: 注文番号=${orderNumber}, 金額=${amount}`,
		);
