 ...
 * @param text 送信するテキスト
 */
export const sendTextMessage = async (userId: string, text: string): Promise<void> => {
  try {
 ...
 */
export const sendMessages = async (userId: string, messages: Message[]): Promise<void> => {
  try {
 ...
 */
export const sendFlexMessage = async (userId: string, altText: string, contents: any): Promise<void> => {
  try {
    const message = {
      type: 'flex',
      altText: altText,
